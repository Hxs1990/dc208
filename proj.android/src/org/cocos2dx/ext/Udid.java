...
//		return SHA1(uid + "uuid");
	...
	public final static String SAVE_TO_CLOUD_PRE = "user_preferences_new";
	public static void saveDataToCloud(){
		String uuid = getUid();		
		SharedPreferences pref = Jni.getGameActivity().getSharedPreferences(SAVE_TO_CLOUD_PRE, Context.MODE_PRIVATE);
    	final Editor e = pref.edit();
    	String model = android.os.Build.MODEL;
		e.putString(model, uuid);
		e.commit();
		Native.nativeSetConfig("saveDataToCloud", uuid);
		BackupManager bm = new BackupManager(Jni.getGameActivity());
		bm.dataChanged();
		
		Log.d("save data to cloud", uuid);
	}
	...
	public static void getDataFromCloud(){
		...
		RestoreObserver observer = new RestoreObserver() {
			...
			public void restoreFinished(int error){
				...
				String uuid = "";
				if(error == 0){
					uuid = getUidFromCloudSharedPreferences();
					if(uuid.equals("")){
						...
						Log.d("data backup get back", uuid);
					}
				}else{
					uuid = String.valueOf(error);
					Log.d("data backup get back", uuid);
				}
				Native.nativeSetConfig("getDataFromCloud", uuid);
			}
		};
	...
	public static String getUidFromCloudSharedPreferences(){
		...
		SharedPreferences pref = Jni.getGameActivity().getSharedPreferences(SAVE_TO_CLOUD_PRE, Context.MODE_PRIVATE);
		String model = android.os.Build.MODEL;
		if ((pref != null) && (pref.getString(model, "") != null) && (pref.getString(model, "").trim().length() > 0)) {
			result = pref.getString(model, "");
			Log.d("data is", result);
		}
		return result;
	}
}
