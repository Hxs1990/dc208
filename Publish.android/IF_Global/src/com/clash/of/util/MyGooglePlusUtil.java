...
//		.setScopes(Scopes.PLUS_LOGIN)  // Space separated list of scopes
	...
	GoogleApiClient.OnConnectionFailedListener {
		...
		public void onConnected(Bundle bundle) {
			...
			final String textShow = accountName + " is connected.";
			...
			GameContext.getActivityInstance().runOnUiThread(new Runnable() { 
				public void run() {
					Toast.makeText(GameContext.getActivityInstance(), textShow, Toast.LENGTH_LONG).show();					
				}
			});
	        //Toast.makeText(GameContext.getActivityInstance(), accountName + " is connected.", Toast.LENGTH_LONG).show();
	        GameContext.getActivityInstance().runOnUiThread(new Runnable() {
				public void run() {
					try {
						final String tempName = Games.getCurrentAccountName(mGoogleApiClient);
                        IF.getInstance().runOnGLThread(new Runnable() {
                            public void run() { 
                            			nativeSetIsLogin(true,tempName);
                                    }
                                });
