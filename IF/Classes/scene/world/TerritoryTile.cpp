...
//    return AddFavorite::create(m_cityInfo.parentCityIndex,m_title->getString());
    string str = _lang("115392");
    string  strAlliance = "(" + m_cityInfo.m_aArea_simpleName+")";
    str = strAlliance + str;
    return AddFavorite::create(m_cityInfo.parentCityIndex,str);
}
...
void TerritoryTile::onClickButton(CCObject * pSender, Control::EventType pCCControlEvent) {
    ...
    switch (buttonState) {
        ...
        case ButtonBuilding:{
            if (soldierAlready >= soldierMax && soldierMax != 0) {
                ...
                if (dataBack) {
                    ...
                    YuanJunTipView::openYuanYunView(m_cityInfo.parentCityIndex, 14, 0);
                }
            }
        }
            break;
        ...
        case ButtonFunction: {
            ...
            SoundController::sharedSound()->playEffects(Music_Sfx_click_button);
            PopupViewController::getInstance()->addPopupInView(TerritoryFunctionView::create(WorldController::getInstance()->m_cityInfo[index].m_allianceAreaInfo.state));
        }
            break;
        case ButtonGoHome: {
            auto &selfMarch = WorldController::getInstance()->m_selfMarchUuid;
