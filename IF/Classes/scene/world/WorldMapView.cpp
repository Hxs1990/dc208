...
void WorldMapView::onTouchEnded(cocos2d::CCTouch *pTouch, cocos2d::CCEvent *pEvent) {
    if(isTouchInside(UIComponent::getInstance()->m_miniBG, pTouch)){
        if(!isMiniMapModel()){
            ...
            }else if (DynamicResourceController::getInstance()->checkMinimapViewResource()) {
...
//            }
        ...
        if(info.marchType == MethodScout || info.marchType == MethodChristmasMarch || info.marchType == MethodDeal || info.marchType == MethodWarehouse || info.targetType == ResourceTile || info.targetType == tile_superMine || info.targetType == Resource_new || info.targetType == tile_tower || info.targetType == tile_banner || info.targetType == tile_wareHouse){
            if(m_touchTroopNode->getChildByTag(info.marchTag)){
...
void WorldMapView::showTroopInfo(const string  marchId) {
 //   CCLOG("march info uid:%s", marchId.c_str());
    ...
    auto view = dynamic_cast<MinimapView*>(PopupViewController::getInstance()->getCurrentPopupView());
...
//lixu 20151215 not sure about what this sprite does, comments it out and nothing happens
//        CCSprite* tempBatchSpNode = NULL;
        ...
        case CityTile:{
            ...
            */
            ...
            if(info.parentCityIndex == info.cityIndex){
                ...
                auto lv = CCLabelBatch::create(CC_ITOA((int)playerCityLv),m_labelNode);
                lv->setScale(0.6*MapGlobalScale);
                lv->setPosition(fixPos+pos);
                cityitem.push_back(lv);
                ...
                if (player.type == PlayerSelf) {
                    ...
                    name->setColor(ccc3(0, 253, 255));
                    if (player.playerVType == 1) {
                        name->setColor(ccc3(22, 253, 252));
                    }
                }else if(GlobalData::shared()->serverType==SERVER_BATTLE_FIELD && info.srcServer>=0 && info.srcServer != GlobalData::shared()->playerInfo.crossFightSrcServerId){
                    name->setColor(ccc3(255, 0, 0));
                    if (player.playerVType == 1) {
                        name->setColor(ccc3(255, 72, 72));
                    }
                }else if(GlobalData::shared()->serverType<=SERVER_TEST && info.srcServer>=0 && info.srcServer != GlobalData::shared()->playerInfo.selfServerId && GlobalData::shared()->playerInfo.crossFightSrcServerId != info.srcServer){//王国远争 本服看到迁服到的人
                    name->setColor(ccc3(255, 0, 0));
                }else if(info.srcServer<0 && CCCommonUtils::isKuaFuWangZhan() && GlobalData::shared()->playerInfo.currentServerId == GlobalData::shared()->playerInfo.selfServerId && GlobalData::shared()->playerInfo.selfServerId != GlobalData::shared()->playerInfo.crossFightSrcServerId){//王国远争 跨服的人，看到迁到服的人
                   name->setColor(ccc3(255, 0, 0));
                }else {
                    name->setColor(ccc3(242, 237, 222));
                }
                ...
                name->setPosition(ccp(0+alNameWidth/2, -15)+pos);
                m_unBatchLabelNode->addChild(name);
                cityitem.push_back(name);
...
std::string WorldMapView::getSpriteName(WorldCityInfo &info){
    ...
    if(info.cityType == MonsterTile){
        ...
        picName = string("00")+CC_ITOA(index + 12)+".png";
    }
        ...
        picName = string("00")+CC_ITOA(index + 22)+".png";
    }
...
void WorldMapView::onShake(CCObject* pObj){
    if (GlobalData::shared()->shakeFlag == 1
        ...
        GameController::getInstance()->callXCApi("action=shakeNum");
