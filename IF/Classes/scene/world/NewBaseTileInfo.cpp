...
//    this->m_resourceNode->setVisible(false);
    ...
    if(m_cityInfo.cityType != ResourceTile && m_cityInfo.cityType != Tile_allianceArea && m_cityInfo.cityType != tile_superMine && m_cityInfo.cityType != tile_tower && m_cityInfo.cityType != tile_wareHouse && m_cityInfo.cityType != tile_banner && m_cityInfo.cityType != Resource_new){
    ...
    if(m_addNode && GlobalData::shared()->serverType==SERVER_DRAGON_BATTLE){
        m_addNode->setVisible(false);
    }
    isInitEnd = false;
    m_openNum = 0;
