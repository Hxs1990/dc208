...
bool FunBuild::initFunBuild(int itemId, CCLabelBatchNode* nameLayer)
{
    ...
    if(itemId > 1000)  //ljf, 这代表已经创建过的建筑，itemId实际上是ItemSpec id="9990"这个字段
    {
        ...
        //--test
//        if(m_buildingKey/1000 == FUN_BUILD_CELLAR ){
//            if(FunBuildController::getInstance()->getMainCityLv() < 25){
//                return true;
//            }
//        }
...
//    if ((m_buildingKey/1000 == FUN_BUILD_QIJI || m_buildingKey < 1000)&&!FunBuildController::getInstance()->building_Miracle_open) {
//        return ;
//    }
    parentX = x;
    parentY = y;
...
void FunBuild::setTime(int times, bool updateLv)
{
    ...
    if (m_lvLabel != NULL && (times<=0 || updateLv)) {
        onCheckOutPut();
        m_lvLabel->setString(CC_ITOA(m_info->level));
        if(m_info->starNum >= 1){
//        if(m_info->level >= 29){
            m_lvLabel->setString(CC_ITOA(m_info->starNum));
        }
    }
...
void FunBuild::onTouchEnded(cocos2d::CCTouch *pTouch, cocos2d::CCEvent *pEvent)
{
    ...
    //...
//    if(m_buildingKey/1000 == FUN_BUILD_CELLAR ){
//        if(FunBuildController::getInstance()->getMainCityLv() >=25 && FunBuildController::getInstance()->getMainCityLv() < 30 ){
//            YesNoDialog::showYesDialog("sdfsdfs");
//            return ;
//        }
//    }
...
//            }
            ...
            m_ptArray->addControlPoint(ccp(-200, 100));
            for (int i=0; i<4; i++) {
                auto person = Person::create(parentX+10+120+i*20, parentY+5+0+i*(-10), m_batchNode, zOrder*1000+tmpOrd, m_ptArray, 10.0);
                person->onSetVisible(0);////可以控制是否显示
                m_personArray->addObject(person);
                tmpOrd++;
            }
            for (int i=0; i<4; i++) {
...
void FunBuildState::onGetBtnClick(CCObject * pSender, Control::EventType pCCControlEvent)
{
    if (m_getNode->isVisible() && m_parFinish) {
        ...
        for (int j=0; j<=2; j++) {
            auto particle = ParticleController::createParticle(CCString::createWithFormat("ButtonGet_%d",j)->getCString());
            particle->setPosition(CCPointZero);
            parNode->addChild(particle);
        }
        parNode->setTag(9999);
        parNode->setScale(0.6);
        parNode->setPosition(ccp(0,44));
        m_getNode->addChild(parNode);
        ...
        this->scheduleOnce(schedule_selector(FunBuildState::removeGetParNode), 0.2);
    }
}
...
{
    Node* parNode = m_getNode->getChildByTag(9999);
