...
bool ImperialScene::init()
{
    ...
    //统计数据用，以后删除
#if CC_TARGET_PLATFORM==CC_PLATFORM_ANDROID
    if(CCCommonUtils::isUserNewUid() && saveCounter == 0){
        string _gameUid = CCUserDefault::sharedUserDefault()->getStringForKey(GAME_UID, "");
        std::string dataFromCloud = CCUserDefault::sharedUserDefault()->getStringForKey("getDataFromCloud", "");
        if(dataFromCloud != ""){
            NetController::shared()->getFromCloud(dataFromCloud, _gameUid);
        }
        
        std::string saveDataToCloud = CCUserDefault::sharedUserDefault()->getStringForKey("saveDataToCloud", "");
        if(saveDataToCloud != ""){
            NetController::shared()->saveToCloud(dataFromCloud, _gameUid);
        }
...
//    waterSpr->setAnchorPoint(ccp(0, 0));
//    waterSpr->setPosition(ccp(0,0));
//    waterSpr->setContentSize(CCSizeMake(sumWidth, sumHight));
//    m_touchLayer->addChild(waterSpr, 70);
...
//        ccbName = "ImperialCities_2_old";
    ...
    m_touchLayer->addChild(m_nightLights, 2002);

    m_cloudLayer = CCLayer::create();
    m_cloudLayer->setName("m_cloudLayer");
    m_touchLayer->addChild(m_cloudLayer,2070);
    
//    CCCommonUtils::makeTwoAnimatSpr(m_dragon, "Dragon_%d.png", 5, 4, 0.1);
    ...
    m_touchLayer->addChild(m_dragonFlyNode, 2080);
...
//    m_flyArrow->setFlipY(true);
    ...
    m_touchLayer->addChild(m_sSqerTouchNode,1996);//////层级设为1996
...
//    float minZoomScale = m_viewPort->getMinZoom() * 1.4;
    ...
    if (FunBuildController::getInstance()->oldx == 99999 || (GlobalData::shared()->playerInfo.level==1 && GlobalData::shared()->playerInfo.exp==0)) {
        ...
        if (GlobalData::shared()->playerInfo.level==1 && GlobalData::shared()->playerInfo.exp==0 && (gFake=="" || gFake=="start_1")) {
            if ( false && !CCCommonUtils::isTestPlatformAndServer("guide_skip_2") ) {
...
//    m_touchLayer->addChild(m_resBlentBatchNode,199);
    ...
    if(true) {
        ...
        auto bdBatch0 = CCSpriteBatchNode::createWithTexture(resSp->getTexture());
        m_buildBatchMap[0] = bdBatch0;
        m_touchLayer->addChild(bdBatch0,202);
    }
...
//        m_bRunning = true;
    ...
    //喷泉上面的瓶子
        if (GlobalData::shared()->isOpenAnniversary) {//GlobalData::shared()->isOpenAnniversary
            if(GlobalData::shared()->isXiaoMiPlatForm()){
                return;
            }
            m_fountainNode->removeChildByTag(ACTIVITY_BOTTLE_TAG);
            CCSprite *fSpr =CCLoadSprite::createSprite("anniversay_tubiao.png");//anniversay_tubiao.png
            fSpr->setTag(ACTIVITY_BOTTLE_TAG);
            fSpr->setPosition(CCPoint(70, 170));
            m_fountainNode->addChild(fSpr);
            ...
            CCDelayTime* dt = CCDelayTime::create(3.0f);
            CCSequence* seq = CCSequence::create(rt1, rt2, rt3, rt4, rt5, dt, NULL);
            fSpr->runAction(CCRepeatForever::create(seq));
            for (int j=1; j<=4; j++) {
                auto sprParticle = ParticleController::createParticle(CCString::createWithFormat("bottle_%d",j)->getCString());
                sprParticle->setPosition(CCPoint(47, 47));
                fSpr->addChild(sprParticle);
            }
        }
...
{
    string particleName = "snow"; // "snow", "rain"
    m_isRain = true;
    auto winSize = CCDirector::sharedDirector()->getWinSize();
...
//                PopupViewController::getInstance()->addPopupInView(SuggestionView::create());
        ...
        if (st && m_beginTouchType==1 && isTouchInside(m_princessNode, pTouch) && m_princessNode->isVisible()) {
            ...
            if (FunBuildController::getInstance()->TalkViewContent!="" && princessNodeOnce == false) {
                ...
                GameController::getInstance()->callXCApi("PrincessTalk");
        ...
        if (st && isTouchInside(m_fountainNode->getChildByTag(ACTIVITY_BOTTLE_TAG), pTouch)) { // && m_fountainNode->getChildrenCount()<=0
            st = false;
//            m_fountainNode->removeChildByTag(ACTIVITY_BOTTLE_TAG);
...
void ImperialScene::onQijiBottomShow(int lv){
    if(FunBuildController::getInstance()->getMainCityLv()>=lv) {
            ...
            m_qijiBuild->setNamePos(m_qijiNode->getPositionX(), m_qijiNode->getPositionY(), m_signLayer, m_arrbatchNode, m_resbatchNode, hod);
    }
}
...
CCNode* ImperialScene::getBuildById(int itemId){
    if (itemId == SPE_BUILD_NEWED || itemId == SPE_BUILD_MERCHANTE || itemId == SPE_BUILD_GOLD_BOX) {
...
//        if(m_noticeCount>0){
//            //有提示
//            m_mailBuild->getAnimationManager()->runAnimationsForSequenceNamed("Loop");
//        }else {
//            //没有提示
//            m_mailBuild->getAnimationManager()->runAnimationsForSequenceNamed("Default");
//        }
...
//    CCB_MEMBERVARIABLEASSIGNER_GLUE_WEAK(this, "m_bg6", CCNode*, this->m_bg6);
    ...
    CCB_MEMBERVARIABLEASSIGNER_GLUE_WEAK(this, "m_vikingParentNode", CCNode*, this->m_vikingParentNode);
...
//    }
    ...
    if(false) {// GlobalData::shared()->playerInfo.gmFlag==1 关闭圣诞树
        m_chrTreeBuild = SpeBuild::create(SPE_BUILD_CHR_TREE);
        m_chrTreeNode->addChild(m_chrTreeBuild);
        int hod = m_chrTreeNode->getZOrder();
        m_chrTreeBuild->setNamePos(m_chrTreeNode->getPositionX(), m_chrTreeNode->getPositionY(), m_signLayer, m_arrbatchNode, m_chrTreeBatchNode, hod, m_chrTreeBlentBatchNode);
    }
...
void ImperialScene::onPrincessAniFuc(CCObject* obj)
{
    ...
    if (integerNum) {
        ...
        if (num == 1) {
            spTrackEntry* entry = m_princessAni->setAnimation(0, "dj", true);
        }else if (num == 2) {
            spTrackEntry* entry = m_princessAni->setAnimation(0, "zl", true);
        }else if (num == 3) {
            spTrackEntry* entry = m_princessAni->setAnimation(0, "zl_1", true);
        }
    }
}
...
//            m_princessShotAni->setTimeScale(entry->endTime/2);
...
void ImperialScene::showPrincessByMsg(CCObject *obj){
    if (CCCommonUtils::isTestPlatformAndServer("Npc_princess") && !FunBuildController::getInstance()->IsHaveRecordeByKey(CLIENT_RECORD_PRINESS) && m_princessWalkNode==NULL && !GuideController::share()->isInTutorial() && m_curGuideEnd == false){
...
{
    if (CCCommonUtils::isTestPlatformAndServer("Npc_princess") && !FunBuildController::getInstance()->IsHaveRecordeByKey(CLIENT_RECORD_PRINESS) && m_princessWalkNode==NULL && !GuideController::share()->isInTutorial()){
...
{
    if (CCCommonUtils::isTestPlatformAndServer("Npc_princess") && !FunBuildController::getInstance()->IsHaveRecordeByKey(CLIENT_RECORD_PRINESS) && m_princessWalkNode==NULL && !GuideController::share()->isInTutorial() && m_curGuideEnd == false && m_princessWalkStart == false) {
...
void ImperialScene::onShowPrincessWalk(float dt)
{
        ...
        CCFileUtils::sharedFileUtils()->isFileExist("Imperial/Imperial_30.atlas"))
    {
        ...
        spTrackEntry* entry = m_princessWalkAni->setAnimation(0, "walk", true);
...
void ImperialScene::onStopPrincessWalk() {
    ...
    if (m_princessWalkAni) {
        m_princessWalkAni->setToSetupPose();
        spTrackEntry* entry = m_princessWalkAni->setAnimation(0, "waiting", true);
    }
...
{
//    int size = 20;
//    m_dragon->setAnchorPoint(ccp(0,0));
//    m_dragon->setScale(1.0);
//    Vector<SpriteFrame*> myArray;// = new CCArray();
//    Vector<SpriteFrame*> myArray1;// = new CCArray();
//    CCSpriteFrame *myframe[size];
//    for (int i=0; i<size; i++) {
//        myframe[i] = CCLoadSprite::loadResource(CCString::createWithFormat("Dragon_%d.png", i+1)->getCString());
//        myframe[i]->getTexture()->setAntiAliasTexParameters();
//        myArray.pushBack(myframe[i]);
//    }
//    m_dragon->setDisplayFrame(myframe[0]);
...
//
//    
//    CCRepeat* repet = CCRepeat::create(animate, 1);//+rand()%2
...
void ImperialScene::updateVikingsShipLock(int seq, bool isShow)
{
    ...
    if(isShow == true)
    {
        ...
        if (!pNode)
        {
            ...
            //pNode->setGlobalZOrder(1);
            pNode->setPosition(13, 34);
            posCCBNode->addChild(pNode, 1);
...
void ImperialScene::onCreateBridge()
{
    ...
    m_touchLayer->setCameraMask((unsigned short)CameraFlag::USER4, true);
    m_node3d->setCameraMask((unsigned short) CameraFlag::USER2, true);
    m_bridge3D_Up->setCameraMask((unsigned short) CameraFlag::USER2, true);
    m_bridge3D_Down->setCameraMask((unsigned short) CameraFlag::USER2, true);
}
