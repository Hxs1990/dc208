...
{
    GameController::getInstance()->callXCApi("action=lottery_share_alliance_click");
    if (GlobalData::shared()->playerInfo.isInAlliance())
    {
        ...
        cmd->sendAndRelease();
        GameController::getInstance()->callXCApi("action=lottery_share_alliance");
        closeSelf();
    } else {
...
void LotteryShareConfirmView::onClickKingShareBtn(Ref* pSender, Control::EventType event)
{
    ...
    cmd->putParam("lotteryInfo", __String::create(info));
    cmd->sendAndRelease();
    GameController::getInstance()->callXCApi("action=lottery_share_king");
    closeSelf();
}
