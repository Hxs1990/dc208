...
//    if (m_nb_bg2) {
//        m_nb_bg2->setContentSize(m_eventNode->getContentSize());
//        m_nb_bg2->setPosition(m_eventNode->getPosition());
//    }
    ...
    for (int i=0; i<num; i++) {
        if(AllianceDailyController::getInstance()->isSwitchOpen()){
            if (i==0) {
                ...
                AllianceFunCell* cell = AllianceFunCell::create(_lang(title), m_info,iconStr,m_funList,i+1);
                if(CCCommonUtils::isIosAndroidPad())
                    cell->setPosition(ccp(5, totalH-(i+1+1)*85*2. - 84*2.));
                else
                    cell->setPosition(ccp(5, totalH-(i+1+1)*85 - 84));
                m_scrollView->addChild(cell);
            }else if (i==num-1){
                const char*  title = titles3[i];
                const char*  iconStr = icons3[i];
                ...
                if(CCCommonUtils::isIosAndroidPad())
                    cell->setPosition(ccp(5, totalH-(1+1)*85*2. - 84*2.));
                else
                    cell->setPosition(ccp(5, totalH-(1+1)*85 - 84));
                m_scrollView->addChild(cell);
            }
