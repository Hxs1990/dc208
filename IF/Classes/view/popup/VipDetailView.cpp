...
//    }
    ...
    if(VipUtil::isSVIP() && playerInfo.SVIPLevel>0){
        m_sprBar[2] = CCLoadSprite::createSprite("svip_jindutiao.png");
    }else{
    ...
    if(VipUtil::isSVIP() && playerInfo.SVIPLevel>0){
        m_sprBar[3] = CCLoadSprite::createSprite("svip_jindutiao.png");
    }else{
...
void VipDetailView::onAddVipAnim(){
    ...
    for(int i=1;i<=4;i++){
        string name = "VIPUI_";
        name+=CC_ITOA(i);
        auto praticle1 = ParticleController::createParticle(name);
...
void VipDetailView::refreshPageFlag(){
    if(m_vipType==1){
        if(VipUtil::isSVIP() && m_vipLevel==10){
            ...
            m_vipNodeRight->setVisible(false);
        ...
        m_vipNodeLeft->setVisible(false);
        m_svipNodeLeft->setVisible(true);
        m_vipNodeRight->setVisible(false);
        m_svipNodeRight->setVisible(true);
    }
}
