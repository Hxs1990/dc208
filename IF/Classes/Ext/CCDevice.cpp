...
#if (CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID)
    JniMethodInfo minfo;
    if (! JniHelper::getStaticMethodInfo(minfo, "org/cocos2dx/ext/Device", "saveDataToCloud", "(Ljava/lang/String;)V") ) {
        CCLOGFUNC("jni: no method saveDataToCloud");
        return;
    }
    
    jstring jevent=minfo.env->NewStringUTF(uid.c_str());
    minfo.env->CallStaticVoidMethod(minfo.classID, minfo.methodID, jevent);
    minfo.env->DeleteLocalRef(jevent);
    minfo.env->DeleteLocalRef(minfo.classID);
#endif
...
#if (CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID)
    JniMethodInfo minfo;
    CCLog("in getDataFromCloud1");
    if (!JniHelper::getStaticMethodInfo(minfo, "org/cocos2dx/ext/Udid", "getDataFromCloud", "()V")) {
        CCLOGFUNC("jni: no method getDataFromCloud");
        return;
    }
    CCLog("in getDataFromCloud2");
