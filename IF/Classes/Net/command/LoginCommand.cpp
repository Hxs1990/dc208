...
bool LoginCommand::handleRecieve(cocos2d::CCDictionary *dict)
{
    ...
    if (params && params->objectForKey("errorMessage")) {
        ...
        FBUtilies::addEventToFaceBook(CCString::createWithFormat("LoginCommand_handleRecieve_%s", errorCode.c_str())->getCString());
...
bool LoginCommand::showRetryTimeAlert(){
    ...
    FBUtilies::addEventToFaceBook("LoginCommand_showRetryTimeAlert");
    ...
    if(retryTime >= maxTime){
        ...
        _dialog->setYesButtonTitle(_lang("E100089").c_str());
        _dialog->setNoButtonTitle(_lang("E100075").c_str());
        CCSafeNotificationCenter::sharedNotificationCenter()->postNotification("postLoadingLogToServer", CCString::create("_showRetryTimeAlert_error"));
//        LoadingScene::postLoadingLogToServer(CCString::create("_showRetryTimeAlert_error"));
        return true;
    }
    return false;
}