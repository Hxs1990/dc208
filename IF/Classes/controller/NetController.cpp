...
void NetController::saveToCloud(std::string value, std::string uuid){
    if(value == ""){
        return;
    }
    ...
    std::string type = "savenew";
    std::string time = timeStr;
    std::string param = CCString::createWithFormat("uid=%s&type=%s&time=%s&value=%s",
                                                   uuid.c_str(),
                                                   type.c_str(),
                                                   time.c_str(),
...
void NetController::getFromCloud(std::string value, std::string uuid){
    if(value == ""){
        return;
    }
...
void NetController::getServerStatusBack(CCHttpClient* client, CCHttpResponse* response){
    ...
    }else if (!response->isSucceed())  // 这个时候有可能是设备不能上网的状态，另外一种可能是服务器有问题。
    {
        ...
        FBUtilies::addEventToFaceBook("ServerStatusBack_failed");
...
//        status = 1;  // 只要是进到这个里面，就应该是1，
        if(iter == NULL){
            ...
            try {
                ...
                if(c){
                    ...
                    {
                        FBUtilies::addEventToFaceBook("ServerStatusBack_cokerrcode0");
    ...
    if(status == 1){
        ...
        FBUtilies::addEventToFaceBook("ServerStatusBack_unknow");
...
void NetController::funFeedBack(){
    ...
    if(SceneController::getInstance()->currentSceneId == SCENE_ID_LOADING)
    {
        CCSafeNotificationCenter::sharedNotificationCenter()->postNotification("postLoadingLogToServer");
    }
}
...
//            CCDirector::sharedDirector()->getScheduler()->scheduleSelector(schedule_selector(NetController::checkReConnection), this, 0.5, false);
        }
...
void NetController::sendfacebookEvent()
{
    ...
    FBUtilies::addEventToFaceBook("net_mode_http");
}
