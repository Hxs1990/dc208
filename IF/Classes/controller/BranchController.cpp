...
void BranchController::addToBranchData(std::string cmd, std::string value){
    ...
    if(cmd == "telkomselpromotion"){
        PromotionCommand *cmd = new PromotionCommand(value);
        cmd->sendAndRelease();
    }
}

void BranchController::sendPormotionCmd(){
    if(m_branchData.find("telkomselpromotion") != m_branchData.end()){
        std::string value = m_branchData["telkomselpromotion"];
        if(value == ""){
            return;
        }
        PromotionCommand *cmd = new PromotionCommand(value);
        cmd->sendAndRelease();
        m_branchData["telkomselpromotion"] = "";
    }
}
...
extern "C" {
#endif
   JNIEXPORT void JNICALL Java_org_cocos2dx_ext_Native_nativeSetBranchValue(JNIEnv* env, jobject thiz, jstring key, jstring value) {
